...
export function FinancePage() {
  ...
  const regions = Array.from(new Set(orders.map(({ region: value }) => value).filter(Boolean))) as string[];
  ...
  }), [client, dueAt, installments, method, orders, region, status, supplier, tab]);
  ...
  return (
    <section aria-labelledby="finance-title">
      ...
      <div className="finance-tabs" role="tablist" aria-label="Visões financeiras">
        {tabs.map((item) => <button key={item.value} role="tab" aria-selected={tab === item.value} type="button" onClick={() => setTab(item.value)}>{item.label}</button>)}
      </div>
