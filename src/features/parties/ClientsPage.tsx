...
export function ClientsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return clients.filter((party) => (!term || [party.name, party.contact, party.email].join(" ").toLocaleLowerCase("pt-BR").includes(term)) && (!region || party.region === region));
  ...
  return (
    <section aria-labelledby="clients-title">
      <header className="page-header"><div><span className="page-eyebrow">Relacionamentos</span><h1 id="clients-title">Clientes</h1><p>Contatos, condições e histórico operacional em uma única consulta.</p></div></header>
      <PrototypeNotice />
      <FilterBar label="Filtros de clientes">
        <label>Buscar<input aria-label="Buscar clientes" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nome, contato ou e-mail" /></label>
